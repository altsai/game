...
public class TwoPlayerStartServer extends BasicGameState {
  ...
  public void init(GameContainer gc, StateBasedGame s) throws SlickException {
    ...
    if (!initializedTextFields) {
      serverField1390 = new TextField(
          gc,
          searchFont,
          (1390 / 2 - entryFont.getWidth("Enter server number/name to join: ") - 200)
              / 2 + entryFont.getWidth("Enter server number/name to join: "),
          75 + headerFont.getLineHeight() + 10, 200, entryFont.getLineHeight());

      serverName1390 = new TextField(gc, searchFont,
          (int) (1390 * 1.5 - entryFont
              .getWidth("Enter server name to create: ") - 200)
              / 2
              + entryFont.getWidth("Enter server name to create: "),
          75 + headerFont.getLineHeight() + 10, 200, entryFont.getLineHeight());

      serverField1132 = new TextField(
          gc,
          searchFont,
          (1132 / 2 - entryFont.getWidth("Enter server number/name to join: ") - 200)
              / 2 + entryFont.getWidth("Enter server number/name to join: "),
          75 + headerFont.getLineHeight() + 10, 200, entryFont.getLineHeight());

      serverName1132 = new TextField(gc, searchFont,
          (int) (1132 * 1.5 - entryFont
              .getWidth("Enter server name to create: ") - 200)
              / 2
              + entryFont.getWidth("Enter server name to create: "),
          75 + headerFont.getLineHeight() + 10, 200, entryFont.getLineHeight());
  ...
  private void getServers() throws SQLException {
    ...
    // Set numberWidth
    numberWidth = entryFont.getWidth(Integer.toString(serverNames.size()))
        + PADDING;
  ...
  public void render(GameContainer gc, StateBasedGame s, Graphics g)
    ...
    headerFont.drawString(
        (Window.width / 2 - headerFont.getWidth("Join Server")) / 2,
        currHeight, "Join Server", Color.white);
    ...
    entryFont
        .drawString(
            (Window.width / 2
                - entryFont.getWidth("Enter server number/name to join: ") - 200) / 2,
            currHeight, "Enter server number/name to join: ", Color.white);
    ...
    if (serverNames == null || serverNames.size() == 0) {
      ...
      float tableHeight = Math.min(normalTableHeight,
          (entryFont.getLineHeight() + 5) * serverNames.size());
      ...
      // Separator line
      g.drawLine(tableX + numberWidth, tableY, tableX + numberWidth, tableY
          + tableHeight);
      ...
      for (float y = tableY + 5; y < tableY + tableHeight
          && entryIndex < serverNames.size(); y += entryFont.getLineHeight() + 5) {
        // Draw number
        float x = tableX
            + (numberWidth - entryFont.getWidth(Integer
                .toString(entryIndex + 1))) / 2;
        entryFont.drawString(x, y, Integer.toString(entryIndex + 1),
            Color.white);

        // Draw name
        x = tableX + numberWidth
            + (nameWidth - entryFont.getWidth(serverNames.get(entryIndex))) / 2;
    ...
    currHeight = 75;
    headerFont.drawString(
        (Window.width * 1.5f - headerFont.getWidth("Create Server")) / 2,
        currHeight, "Create Server", Color.white);
    currHeight += (10 + headerFont.getLineHeight());

    // Draw serverField
    entryFont.drawString(
        (Window.width * 1.5f
            - entryFont.getWidth("Enter server name to create: ") - 200) / 2,
        currHeight, "Enter server name to create: ", Color.white);
    g.setColor(Color.white);
    ...
    for (; n.hasMoreElements();) {
      ...
      for (; a.hasMoreElements();) {
        ...
        if (addr.isSiteLocalAddress()) {
          // this returns the first one found.... maybe it'll work for all
          // cases?
  ...
  private void addServer(String name, String localAddr, int width, int height,
      String id) throws SQLException, SocketException {
    ...
    if (serverFieldUsing.hasFocus()
      ...
      try {
        ...
        if (serverNum - 1 < serverAddresses.size() && serverNum > 0) {
          ...
          handleSizing(serverWidths.get(serverNum - 1),
              serverHeights.get(serverNum - 1), serverIds.get(serverNum - 1));
          s.getState(States.TWO_PLAYER_CLIENT).init(gc, s);
          s.enterState(States.TWO_PLAYER_CLIENT);
        } else {
          Window.showMessage("Invalid Number",
              "Oops! That's not a valid number!");
        }
      } catch (NumberFormatException e) {
        if (serverNames.contains(input)) {
          address = serverAddresses.get(serverNames.indexOf(input));
          handleSizing(serverWidths.get(serverNames.indexOf(input)),
              serverHeights.get(serverNames.indexOf(input)),
              serverIds.get(serverNames.indexOf(input)));
          s.getState(States.TWO_PLAYER_CLIENT).init(gc, s);
          s.enterState(States.TWO_PLAYER_CLIENT);
        } else {
    ...
    if (gc.getInput().isKeyPressed(Input.KEY_ENTER)
      ...
      try {
        ...
        if (!serverNames.contains(input)) {
          ...
          addServer(serverNameUsing.getText(), address, Window.width,
              Window.height, serverId);
          s.getState(States.TWO_PLAYER_HOST).init(gc, s);
          s.enterState(States.TWO_PLAYER_HOST);
        } else {
          Window.showMessage("Name Taken",
              "Oops! That server name is already taken!");
        }
