...
public class SinglePlayerGameState extends BasicGameState {
   ...
   * @param delta  Int, change in time since last update
   */
  private void updateAndCheckCollisions(GameContainer gc
      , StateBasedGame s
      , int delta) {
  ...
  private void spawnZombie() {
    ...
    if (System.currentTimeMillis() - this.lastZombieSpawnTime
      ...
      if (random.nextInt(9) < 3) {
        for (int i = 0; i < this.difficultyLevel; i++) {
          ...
          newZombie.setSpeed(ZOMBIE_BASE_SPEED
              + ((this.difficultyLevel - 1) * SPEED_MULTIPLIER) * ZOMBIE_BASE_SPEED);
      ...
      newZombie.setSpeed(ZOMBIE_BASE_SPEED
          + ((this.difficultyLevel - 1) * 0.1) * ZOMBIE_BASE_SPEED);
      this.entities.add(new Zombie(this.player1));
  ...
  private void endGame(StateBasedGame s) {
    s.enterState(States.SINGLE_PLAYER_END_GAME, new FadeOutTransition(), new FadeInTransition());
  }
